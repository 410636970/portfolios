...
public class fatherController : MonoBehaviour
{
    ...
    public static float numbersize = 0.5f;
    private float h = 0;
    private float v = 0;
    ...
    void OnTriggerEnter2D(Collider2D coll)//chapter one
    {
        if (coll.gameObject.tag == "controllfalse")
        {
    ...
    void numberchange1()
    {
        Vector3 scale = number.transform.localScale;
        scale.x = numbersize;
